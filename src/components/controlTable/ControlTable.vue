<template>
  <div class="wrapper">
    <v-container class="d-flex flex-wrap flex-column ma-0 pa-0" fluid>
      <v-row>
        <v-col cols="12" sm="6">
          <v-combobox
            ...
            hide-selected
            :hint="
              'If you can see this hint, it probably means you\'re typing a filter that won\'t work'
            "
            :items="
              [{header: 'Select an option or type one and hit enter'}].concat(
                controls
              )
            "
          ...
          <v-sheet :width="'100%'" :height="'70vh'">
            <c-grid
              ...
              :underlay-background-color="
                this.$vuetify.theme.dark ? 'black' : 'white'
              "
              ...
              <template slot="layout-header">
                ...
                <c-grid-layout-row>
                  <c-grid-header
                    ...
                    >{{
                      data
                        .filter((control) => control[col.value])
                        .length.toString()
                    }}</c-grid-header
                  >
                </c-grid-layout-row>
      ...
      <v-row>
        <v-col>
          <v-card flat outlined class="pa-2 mb-6">
            <v-row dense>
              <v-col>
                <v-card class="d-flex justify-center" dense flat tile
                  >Legend</v-card
                >
                <v-divider class="" />
              </v-col>
